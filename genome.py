...
class Genome():
    ...
    def crossover(g1, g2):
        ...
        return new_genome[:len(g1)]

    @staticmethod
    def point_mutate(genome, rate, amount):
        new_genome = np.copy(genome)
        mutation_mask = np.random.rand(*new_genome.shape) < rate
        mutation_values = np.random.normal(0, amount, new_genome.shape) * mutation_mask
        new_genome += mutation_values
        new_genome = np.clip(new_genome, 0, 0.9999)  # Keep values within range
        return new_genome
    ...
    def shrink_mutate(genome, rate):
        ...
        indices = np.random.choice(len(genome), num_to_remove, replace=False)
        
        new_genome = np.delete(genome, indices, axis=0)
        return new_genome if len(new_genome) > 0 else np.copy(genome)  # Prevent empty genome

    @staticmethod
    def grow_mutate(genome, rate):
        max_genome_length = 50
        if len(genome) >= max_genome_length or random.random() >= rate:
            return np.copy(genome)  # No growth if genome is at max size
        
        num_to_add = max(1, int(rate * len(genome)))  # Grow based on rate
        new_genes = np.array([Genome.get_random_gene(len(genome[0])) for _ in range(num_to_add)])
        
        insert_idx = np.random.randint(0, len(genome)+1)  # Random insertion
        new_genome = np.insert(genome, insert_idx, new_genes, axis=0)
        
        return new_genome[:max_genome_length]  # Ensure genome doesn't exceed max length
