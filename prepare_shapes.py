...
def generate_noise(x, y, scale, octaves, fade, stretch):
        noise_value = 0
        amplitude = 1.0
        frequency = scale
        for _ in range(octaves):
            noise_value += pnoise2(x * frequency, y * frequency) * amplitude
            amplitude *= fade
            frequency *= stretch
        return noise_value

def generate_mountain(filename, size=10, resolution=0.5, sigma=3, height=5, noise_scale=0.5, noise_factor=1.0, octaves=4, fade=0.5, stretch=2.0):
    vertices = []
    faces = []
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            z_gaussian = gaussian2(x, y, sigma, height)
            z_noise = generate_noise(x, y, noise_scale, octaves, fade, stretch) * noise_factor
            z = z_gaussian + z_noise
            vertices.append([x, y, z])

    for i in range(grid_size - 1):
        for j in range(grid_size - 1):
            bottom_left = i * grid_size + j
            bottom_right = i * grid_size + j + 1
            top_left = (i + 1) * grid_size + j
            top_right = (i + 1) * grid_size + j + 1
...
# Example usage:
generate_mountain('./shapes/roots.obj', size=20, resolution=0.5, sigma=2, height=8, noise_scale=0.3, noise_factor=2.0)
